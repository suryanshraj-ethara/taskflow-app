...
def seed():
    ...
    ankit = User.objects.create_user(
        'ankit', 'ankit@todoo.com', 'ankit123',
        first_name='Ankit', last_name='Sharma', role='MEMBER'
    )
    satyendra = User.objects.create_user(
        'satyendra', 'satyendra@todoo.com', 'satyendra123',
        first_name='Satyendra', last_name='Kumar', role='MEMBER'
    )
    saumya = User.objects.create_user(
        'saumya', 'saumya@todoo.com', 'saumya123',
        first_name='Saumya', last_name='Singh', role='MEMBER'
    )
    manu = User.objects.create_user(
        'manu', 'manu@todoo.com', 'manu123',
        first_name='Manu', last_name='Verma', role='MEMBER'
    )
    sumit = User.objects.create_user(
        'sumit', 'sumit@todoo.com', 'sumit123',
        first_name='Sumit', last_name='Gupta', role='MEMBER'
    )
    priya = User.objects.create_user(
        'priya', 'priya@todoo.com', 'priya123',
        first_name='Priya', last_name='Patel', role='MEMBER'
    )
    sneha = User.objects.create_user(
        'sneha', 'sneha@todoo.com', 'sneha123',
        first_name='Sneha', last_name='Mishra', role='MEMBER'
    )
    dhananjay = User.objects.create_user(
        'dhananjay', 'dhananjay@todoo.com', 'dhananjay123',
        first_name='Dhananjay', last_name='Rao', role='MEMBER'
    )
    vaibhav = User.objects.create_user(
        'vaibhav', 'vaibhav@todoo.com', 'vaibhav123',
        first_name='Vaibhav', last_name='Joshi', role='MEMBER'
    )
    shreya = User.objects.create_user(
        'shreya', 'shreya@todoo.com', 'shreya123',
        first_name='Shreya', last_name='Das', role='MEMBER'
    )
    ...
    talos.members.add(ankit, satyendra, saumya, manu)  # Team Alpha
    ...
    valor.members.add(sumit, priya, sneha)  # Team Beta
    ...
    atlas.members.add(dhananjay, vaibhav, shreya)  # Team Gamma
    ...
    vindex.members.add(ankit, priya, dhananjay, shreya)
    ...
    Task.objects.create(title='API security audit', description='Review and fix API vulnerabilities', priority='HIGH', status='TODO', due_date=timezone.now().date() - timedelta(days=1), project=valor, assigned_to=sumit)
    ...
    Task.objects.create(title='Write documentation', description='User guides and API docs', priority='LOW', status='TODO', due_date=timezone.now().date() + timedelta(days=15), project=atlas, assigned_to=dhananjay)

    # --- Tasks for Vindex ---
    Task.objects.create(title='Research NLP models', description='Evaluate BERT vs GPT for semantic search', priority='HIGH', status='IN_PROGRESS', due_date=timezone.now().date() + timedelta(days=12), project=vindex, assigned_to=ankit)
    Task.objects.create(title='Design search result ranking algorithm', description='Relevance scoring system', priority='MEDIUM', status='TODO', due_date=timezone.now().date() + timedelta(days=20), project=vindex, assigned_to=priya)
    Task.objects.create(title='Build document indexing service', description='Elasticsearch integration', priority='HIGH', status='TODO', due_date=timezone.now().date() + timedelta(days=25), project=vindex, assigned_to=dhananjay)

    print("=" * 50)
    print("SEED DATA CREATED SUCCESSFULLY!")
    print("=" * 50)
    print()
    print("CREDENTIALS (All saved in Django Admin):")
    print("-" * 50)
    ...
    print("  Team Beta  (Valor): Sumit, Priya, Sneha")
    print("  Team Gamma (Atlas): Dhananjay, Vaibhav, Shreya")
    print()
    print("Django Admin URL: http://localhost:8000/admin/")
    print("Admin login: suryansh / suryansh123")
